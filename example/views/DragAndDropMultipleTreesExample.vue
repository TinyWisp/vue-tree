  ...
  <div class="example-wrapper">
    <div class="panel">
      <vue-tree
        ...
        @drop="(dragAndDrop, event) => {drop('leftTree', dragAndDrop, event)}"/>
      <vue-tree
        ref="rightTree" 
        class="tree right"
        treeId="rightTree"
        ...
        :enableDragNodeOut="true"
        :enableDropExternalElement="true"
        @drop="(dragAndDrop, event) => {drop('rightTree', dragAndDrop, event)}"/>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    drop (thisTreeId, dragAndDrop, event) {
      if (dragAndDrop.status !== 3) {
        return
      }

      let obj = JSON.parse(event.dataTransfer.getData('vue-tree'))
